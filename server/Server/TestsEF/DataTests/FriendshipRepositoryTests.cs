...
namespace TestsEF.DataTests
{
    public class FriendshipRepositoryTests : DataTestsBase
    {
        ...
        {
            repository = CreateFriendshipRepository();
            SeedPlayers(3, emailPrefix: "user", 
                nicknamePrefix: "User", pointsSelector: i => i * 10, emailVerified: true);
        }
        ...
        public async Task CreateFriendRequest_NewRequest_ReturnsTrue()
        {
            ...
            var result = await repository.CreateFriendRequestAsync(p1, p2);
            Assert.True(result);
        ...
        public async Task CreateFriendRequest_Duplicate_ReturnsFalse()
        {
            var (p1, _) = GetPlayerByEmail("user1@test.com");
            var (p2, _) = GetPlayerByEmail("user2@test.com");
            Assert.True(await repository.CreateFriendRequestAsync(p1, p2));
            var duplicate = await repository.CreateFriendRequestAsync(p1, p2);
        ...
        public async Task GetPendingRequests_ForAddressee_ReturnsList()
        {
            ...
            await repository.CreateFriendRequestAsync(requesterId, addresseePlayerId);

            var pending = await repository.GetPendingRequestsAsync(addresseeUserAccountId);
        ...
        public async Task RespondToFriendRequest_Accept_SetsStatusToAccepted()
        {
            var (p1, _) = GetPlayerByEmail("user1@test.com");
            var (p2, _) = GetPlayerByEmail("user2@test.com");
            await repository.CreateFriendRequestAsync(p1, p2);
            var responded = await  repository.RespondToFriendRequestAsync(p1, p2, true);
        ...
        public async Task RespondToFriendRequest_Reject_RemovesRequest()
        {
            ...
            var (p2, _) = GetPlayerByEmail("user2@test.com");
            await repository.CreateFriendRequestAsync(p1, p2);
            var responded = await repository.RespondToFriendRequestAsync(p1, p2, false);
            Assert.True(responded);
        ...
        public async Task GetFriendsByUserAccountId_AfterAcceptance_ReturnsBothSides()
        {
            ...
            await repository.CreateFriendRequestAsync(p1, p2);
            await repository.RespondToFriendRequestAsync(p1, p2, true);

            var friendsOfUser1 = await repository.GetFriendsByUserAccountIdAsync(u1);
            var friendsOfUser2 = await repository.GetFriendsByUserAccountIdAsync(u2);
        ...
        public async Task DeleteFriendship_Existing_ReturnsTrueAndRemoves()
        {
            ...
            var deleted = await repository.DeleteFriendshipAsync(p1, p2);
            Assert.True(deleted);

            using (var verify = NewContext())
            {
                Assert.False(verify.Friendship.Any(f => 
                (f.RequesterId == p1 && f.AddresseeId == p2) || (f.RequesterId == p2 && f.AddresseeId == p1)));
            }
        }
        ...
        public async Task DeleteFriendship_NonExisting_ReturnsFalse()
        {
            ...
            var deleted = await repository.DeleteFriendshipAsync(p1, p3);
